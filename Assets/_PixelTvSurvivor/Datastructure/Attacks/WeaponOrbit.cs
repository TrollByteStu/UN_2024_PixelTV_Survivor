        ...
        { 
            ...
            switch (SpinDirection)
            {
                case Spin.Left:
                    ...
                    /*y*/    math.cos(Time.timeSinceLevelLoad * LevelStats[level].RotationSpeed + (math.PI * 2) / QuantityPerLayer * (i % QuantityPerLayer)) * (LevelStats[level].DistanceFromPlayer / LevelStats[level].LayerQuantity * math.floor((i + QuantityPerLayer) / QuantityPerLayer)), 0) + playerTransform.position;
                    break;
                case Spin.Right:
                    Satellites[i].transform.position = new Vector3(
                    /*x*/    math.sin(Time.timeSinceLevelLoad * LevelStats[level].RotationSpeed + (math.PI * 2) / QuantityPerLayer * (i % QuantityPerLayer)) * (LevelStats[level].DistanceFromPlayer / LevelStats[level].LayerQuantity * math.floor((i + QuantityPerLayer) / QuantityPerLayer)),
